...
class Rels extends WriterPart
{
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\EphenyxShop\PhenyxXls\Worksheet\Worksheet $worksheet, $worksheetId = 1, $includeCharts = false, $tableRef = 1)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        if (($worksheet->getDrawingCollection()->count() > 0) || (count($charts) > 0) || $drawingOriginalIds) {
            ...
            // (! synchronize with \EphenyxShop\PhenyxXls\Writer\Xlsx\Worksheet::writeDrawings)
    ...
    private function writeUnparsedRelationship(\EphenyxShop\PhenyxXls\Worksheet\Worksheet $worksheet, XMLWriter $objWriter, string $relationship, string $type): void
    {
        $unparsedLoadedData = $worksheet->getParentOrThrow()->getUnparsedLoadedData();
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\EphenyxShop\PhenyxXls\Worksheet\Worksheet $worksheet, &$chartRef, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            ...
            if (
                $drawing instanceof \EphenyxShop\PhenyxXls\Worksheet\Drawing
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships(\EphenyxShop\PhenyxXls\Worksheet\Worksheet $worksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    public function writeVMLDrawingRelationships(\EphenyxShop\PhenyxXls\Worksheet\Worksheet $worksheet): string
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
